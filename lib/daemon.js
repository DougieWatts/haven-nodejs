...
class Daemon {
    constructor(hostname, port, user, pass, timeout) {
        ...
        try {
            this._request('get_info');// This line is necessary in order to do the initial handshake between this wrapper and monero-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
        } catch (e) {
            throw new Error('Failed to initialize Daemon: ' + e.message);
        }
    }
}
...
Daemon.prototype.getblock_by_hash = function(hash) {
    let method = 'getblock_by_hash';
    let params = {};
    params.hash = hash;
    return this._request(method);
};
...
Daemon.prototype.getblock_by_height = function(height) {
    let method = 'getblock_by_height';
    let params = {};
    params.height = height || 0;
    return this._request(method, params);
};
