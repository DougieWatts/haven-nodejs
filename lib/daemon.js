...
class Daemon {
    constructor(hostname, port, user, pass, timeout, sslCert = null, sslKey = null, sslCa = null) {
...
Daemon.prototype._request = function (method, params = '', interfaceName = 'json_rpc') {
    let options = {
        ...
        json: {},
        agent: this.useHttps ? new https.Agent({
            keepAlive: true,
            maxSockets: 1,
            cert: this.sslCert,
            key: this.sslKey,
    ...
    if (interfaceName === 'json_rpc') {
        ...
        options.json = params; // Use params directly for non-JSON-RPC requests
    }

    if (params && interfaceName === 'json_rpc') {
    ...
    return request.post(`${protocol}://${this.hostname}:${this.port}/${interfaceName}`, options)
        .then((result) => {
